...
function drawmelikeoneofyourfrenchgirls(jobs,start,end) {
  ...
  var treetagsGroup = tags.groupAll().reduce(
    TreereduceAdd, TreereduceRemove, reduceInitial
  ).value();
  ...
  var hazardtagsGroup = tags.groupAll().reduce(
    HazardreduceAdd, HazardreduceRemove, reduceInitial
  ).value();
  ...
  var propertytagsGroup = tags.groupAll().reduce(
    PropertyreduceAdd, PropertyreduceRemove, reduceInitial
  ).value();
...
function HackTheMatrix(id, unit) {
  ...
  GetJSONfromBeacon(id,start,end, function(jobs) {
    ...
    console.log(jobs)
    document.getElementById("total").innerHTML = "Total Job Count: " + 
      (jobs.Results.length);
