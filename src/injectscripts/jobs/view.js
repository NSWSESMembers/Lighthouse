...
function renderNearestAssets({ teamFilter, activeOnly, resultsToDisplay, cb }) {
  if (!masterViewModel.geocodedAddress.peek().Latitude && !masterViewModel.geocodedAddress.peek().Longitude) {
    ...
    window.postMessage(
      {
        type: 'FROM_PAGE_ASSIGNEDLHQ',
        lhq: masterViewModel.entityAssignedTo.peek(),
      },
      '*',
    );
    ...
    var mapMarkers = [];
    if (!activeOnly && typeof teamFilter == 'undefined') {
      //only show LHQ's when in all mode
      ...
      if (event.data.type) {
        if (event.data.type === 'lhqs') {
          if (event.data.data && event.data.data.features) {
            ...
            event.data.data.features.forEach(function (v) {
              v.distance =
                vincenty.distVincenty(
                  v.properties.POINT_Y,
                  v.properties.POINT_X,
            ...
            for (let i = 0; i < _sortedhqDistances.length; i++) {
              //all of them with some conttrol of count
              ...
              let details = `<div id='lhqPopUp' style="width:250px">\
...
function assetLocationButtonAll() {
  ...
  if (!assetMap) {
    ...
    renderNearestAssets({
      ...
      cb: function () {
        spinner.hide();
        $('#asset-route-warning').html(`Showing nearest 5 assets`);
        $('#asset-route-warning').css('visibility', 'unset');
      },
    });
  }
...
function assetLocationButtonFiltered(bypassUI) {
  ...
  if (!bypassUI) {
    ...
    //skip all the UI and just load and go
...
function return_quickmessagebutton() {
  return (
    <div
      id="lighthouse_instantmessage"
      style="position:relative;display:inline-block;vertical-align:middle;padding-left:3px;padding-right:3px;padding-bottom:5px;"
    >
      <button
        class="btn btn-sm btn-default"
        style="background-color: #e24d00;border-color: #cb4f0f"
        type="button"
          ...
          if (response.responseJSON.Results.length) {
            ...
            $.each(response.responseJSON.Results, function (k, v) {
              ...
              if (
                ...
                !address.StreetNumber.match(/Lot/i) &&
                !v.Address.StreetNumber.match(/Lot/i)
              ) {
