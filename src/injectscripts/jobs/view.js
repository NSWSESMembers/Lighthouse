...
//if ops logs update
masterViewModel.notesViewModel.opsLogEntries.subscribe(function () {
  lighthouseMessageTPlus(
    $('div[data-bind="foreach: opsLogEntries"] .text-muted:not([lhTPlus])')
  );
});

//if messages update
masterViewModel.messagesViewModel.messages.subscribe(function () {
  lighthouseMessageTPlus(
    ...
    if (
      ...
      function addToTime(addMins, where, text) {
        var timeInput = $(where)
          ...
          .parent('div')
          .find(
            "input[data-bind$=\"datetimepicker: $parent.jobTeamStatusEstimatedCompletion, attr: {placeholder: $parent.jobTeamStatusTypeBeingUpdated() ? ($parent.jobTeamStatusTypeBeingUpdated().Id == Enum.JobTeamStatusType.Enroute.Id ? 'ETA' : 'ETC') : '' }\"]"
          );
        var now = moment();
        var current = moment();
        if (timeInput.val() != '') {
...
function lighthouseTasking() {
        ...
        function (k, vv) {
          ...
          if (vv.Team.Id == DOMCallsign && vv.Team.Members.length) {
            ...
            $(msg).click(function () {
              ...
              window.open(
                '/Messages/Create?lhquickrecipient=' +
  ...
  $(
    ...
    if ($(v).parent().find('#untask').length == 0) {
      ...
      DOMCallsign = $(this)[0].parentNode.children[0].children[0].href.split(
        '/'
      )[4];
    ...
    if (typeof masterViewModel.geocodedAddress.peek() != 'undefined') {
      if (
        ...
        window.postMessage(
          {
            ...
            lat: masterViewModel.geocodedAddress.peek().Latitude,
            lng: masterViewModel.geocodedAddress.peek().Longitude,
          },
          '*'
        );
        ...
        if (masterViewModel.jobType.peek().ParentId == 5) {
          ...
          switch (
            avlType //translations for different AVL names
          ) {
            case 'RCR':
              avlType = 'GLR';
              break;
          ...
          $.ajax({
            ...
            url:
              urls.Base +
              '/Api/v1/ResourceLocations/GET?resourceTypes=' +
              avlType,
            ...
            complete: function (response, textStatus) {
              if (textStatus == 'success') {
                ...
                if (_sortedAvlDistances.length > 0) {
                  ...
                  var used = 0;
                  _sortedAvlDistances.forEach(function (v) {
                    //safe way to loop with a limit
                    if (used < maxLength) {
                      used++;
                      $('#nearest-avl-text').text(
                        $('#nearest-avl-text').text() +
                          `${v.CallSign} (${v.distance.toFixed(2)} kms), `
                      );
                    }
                  });
                  $('#nearest-avl-text').text(
                    $('#nearest-avl-text').text().slice(0, -2)
                  ); //trim the comma space from the very end
                } else {
                  $('#nearest-avl-text').text(
                    'No AVL results returned for ' + avlType
                  );
                }
                var t1 = performance.now();
                console.log(
                  'Call to calculate distances from AVLs took ' +
                    (t1 - t0) +
                    ' milliseconds.'
                );
              }
...
function jobView_teamsTasked_completedHiddenSwitch() {
  ...
  // Non-Completed Crews
  $(
    'div.widget > div.widget-content > div[data-bind$="taskedTeams"] > div:not(.teamStatus_complete) > div.row:not(:first-child)'
  ).show();
  // Completed Crews
  $(
...
$(document).on(
  'click',
  'div.widget > div.widget-content > div[data-bind$="taskedTeams"] > div',
  jobView_teamsTasked_showHiddenItem
);
    ...
    dom.each(function (r) {
      ...
      if (
        ...
        if (res && res.length) {
          ...
          $(this).attr('lhTPlus', tPlusText);
          $(this).text(
            $(this).text().replace(res[1], `${res[1]} (${tPlusText})`)
          );
        }
      }
    });
...
if (document.getElementById('FinaliseQuickTextBox')) {
  document.getElementById('FinaliseQuickTextBox').onchange = function () {
    var block = document
      .getElementById('finaliseJobModal')
    ...
    if (newValue !== null) {
      switch (newValue.Name) {
        case 'Storm':
          ...
          var quickText = [
            '',
            'No damage to property, scene safe. Resident to arrange for clean up.',
            'Tree removed and scene made safe.',
            'Roof repaired and scene made safe.',
            'Damage repaired and scene made safe.',
            'Job was referred to contractors who have completed the task.',
            'Council have removed the tree from the road, scene made safe.',
            'Branch/tree sectioned; resident/owner to organize removal',
          ];
        ...
        case 'Search':
          ...
          var quickText = [
            '',
            'All teams complete on search, person found safe and well.',
            'All teams complete on search, nothing found.',
          ];
          for (var i = 0; i < quickText.length; i++) {
            var opt = document.createElement('option');
            opt.text = quickText[i];
  ...
  function SetCategory(id) {
    ...
    $.ajax({
      ...
      data: {
        Id: id,
        Ids: [jobId],
        LighthouseFunction: 'SetCategory',
        userId: user.Id,
      },
      cache: false,
      dataType: 'json',
...
function InstantSectorButton() {
  ...
  $.ajax({
    ...
    url:
      urls.Base +
      '/Api/v1/Sectors/Search?entityIds[]=' +
      user.currentHqId +
      '&statusIds=1&viewModelType=2&pageSize=100&pageIndex=1',
    beforeSend: function (n) {
      n.setRequestHeader('Authorization', 'Bearer ' + user.accessToken);
    },
    cache: false,
      ...
      if (textStatus == 'success') {
        if (response.responseJSON.Results.length) {
          ...
          data.Results.forEach(function (entry) {
            ...
            if (entry.Id != currentsector) {
              //if its not the same as the current sector
              item = (
                <li>
                  <a style='text-align:left' href='#'>
                    {entry.Name}
              ...
              //if it is the same, make it bold.
              item = (
                <li>
                  <a style='text-align:left' href='#'>
                    <b>{entry.Name}</b>
  ...
  function SetSector(sector, currentsector) {
    if (sector.Id != currentsector) {
      ...
      //if its the same, then remove it.
      $.ajax({
        type: 'PUT',
  ...
  $.ajax({
    ...
    complete: function (response, textStatus) {
      if (textStatus == 'success') {
        ...
        if (
          data.JobStatusType.Id == 1 ||
          data.JobStatusType.Id == 2 ||
          data.JobStatusType.Id == 4 ||
          data.JobStatusType.Id == 5
        ) {
          //New or Active or Tasked or Refered
          ReturnTeamsActiveAtLHQ(user.hq, sectorFilter, function (response) {
            if (response.responseJSON.Results.length) {
                    ...
                    $(v.Members).each(function (k, vv) {
                      if (vv.TeamLeader) {
                        item = return_li(
                          v.Id,
                          v.Callsign.toUpperCase(),
                          vv.Person.FirstName + ' ' + vv.Person.LastName,
                          v.TaskedJobCount + ''
                        );
                      }
                    });
              ...
              no_results = (
                <li>
                  <a href='#'>No Active Field Teams</a>
                </li>
              );
...
function submitInstantRadioLog(subject, text, cb) {
  masterViewModel.notesViewModel.OperationsLogManager.CreateEntry(
    jobId,
    null,
    null,
    null,
    subject || 'Instant Radio Log',
    text,
    null,
    null,
    null,
...
function return_lipres(title) {
  return (
    <li style='text-align:left' role='presentation' class='dropdown-header'>
      {title}
    </li>
  );
}
...
function return_quicktaskbutton() {
  return (
    <div
      id='lighthouse_instanttask'
      style='position:relative;display:inline-block;vertical-align:middle;padding-left:3px;padding-right:3px;'
      class='dropdown'
    >
      <button
        ...
        id='lhtaskbutton'
      >
        <i class='fa fa-tasks'></i>Instant Task
  ...
  return (
    ...
    >
      <button
        ...
        id='lhsectorbutton'
      >
        <i class='fa fa-cubes'></i>Instant Sector
        <span class='caret'></span>
      </button>
      <ul class='dropdown-menu scrollable-menu'></ul>
    </div>
  ...
  return (
    ...
    >
      <button
        ...
        id='lhcategorybutton'
      >
        <i class='fa fa-database'></i>Instant Category
        <span class='caret'></span>
      </button>
      <ul class='dropdown-menu scrollable-menu'></ul>
    </div>
...
function return_quickradiologbutton() {
  return (
    <div
      id='lighthouse_instantradiolog'
      style='position:relative;display:inline-block;vertical-align:middle;padding-left:3px;padding-right:3px;'
    >
      <button
        class='btn btn-sm btn-default'
        style='background-color: #837947;border-color: #837947'
        type='button'
        id='lhinstantradiologbutton'
      >
        <i class='fa fa-microphone'></i>Instant Radio Log
      </button>
    </div>
  );
}
        ...
        <div class='modal-content'>
          <div class='modal-header'>
            ...
            <h4 class='modal-title'>
              <i class='fa fa-microphone'></i> Instant Radio Log Entry | Job
            </h4>
          </div>
...
function return_quicketamoment() {
  return <div style='padding-left: 5px' id='quickETAtext'></div>;
}
        ...
        case 'success':
          if (response.responseJSON.Results.length) {
            ...
            $job_view_history_container.html(
              <div>
                {_.map(
                  _.filter(history_rows, function (row) {
                    return row.always_show || row.jobs.length;
                  }),
            ...
            // Show/Hide Handler
            var $job_view_history_toggle_old = $(
              <div class='job_view_history_toggle_old'>
                <span>Show</span> Older Jobs
              </div>
            ).click(function () {
              var $t = $(this);
            ...
            $('fieldset.job_view_history_showtoggle').each(function (k, v) {
              var $v = $(v);
...
function whenAddressIsReady(cb) {
  //when external vars have loaded
  var waiting = setInterval(function () {
    //run every 1sec until we have loaded the page (dont hate me Sam)
    if (
      (typeof masterViewModel !== 'undefined') &
      (masterViewModel.geocodedAddress != 'undefined')
    ) {
      if (
        typeof masterViewModel.geocodedAddress.peek() != 'undefined' &&
        masterViewModel.geocodedAddress.peek() !== null
      ) {
...
function renderSectorFilterCheckBox() {
  return (
    <span class='pull-right h6'>
      ...
      <img
        style='width:16px;vertical-align:top;margin-right:5px;margin-left:5px'
        src={lighthouseUrl + 'icons/lh-black.png'}
      />{' '}
      Instant Filter by Sector
    </span>
  );
}
