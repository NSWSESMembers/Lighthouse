 ...
 * @param {(id: string|number|null, name: string) => void} [deps.onChange]  fired whenever the selection changes (set or cleared), e.g. to persist it
...
export function createEntityPicker({
  ...
  picker.select = async function () {
    ...
    try {
      ...
      set(resolved.Id, resolved.Name || resolved.Code || `HQ ${resolved.Id}`);
    } catch (err) {
  ...
  picker.pickSuggestion = function (suggestion) {
    set(suggestion.Id, suggestion.Name);
    picker.input('');
    picker.suggestions([]);
    picker.suggestionIndex(-1);
    picker.suggestionsDismissed(true);
