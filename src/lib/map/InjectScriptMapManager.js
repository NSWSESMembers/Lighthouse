    ...
    if (data && data.length > 0) {
        for (let i = 0; i < data.length; i++) {
          ...
          if (moment().diff(asset.properties.lastSeen, "days") < 15) {
                ...
                <br><br>
                <p><b>${lat}</b>, <b>${lon}</b></p>
                <p ${moment().diff(asset.properties.lastSeen, "days") > 1 ? "style=color:red" : ''}>
                (last seen: ${asset.properties.lastSeen})<br>
                (last seen: ${moment(asset.properties.lastSeen).fromNow()})
                </p>
                <p>Talkgroup: <b>${asset.properties.talkgroup}</b><br>(last activity: ${asset.properties.talkgroupLastUpdated})<br>
                (last seen: ${moment(asset.properties.talkgroupLastUpdated).fromNow()})</p>
                <p></p>
                <hr>
