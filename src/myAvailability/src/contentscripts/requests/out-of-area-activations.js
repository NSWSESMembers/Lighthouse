          ...
          $.ajax({
            ...
            complete: function (response, textStatus) {
              if (textStatus === 'success') {
                finalList[index].phoneNumbers = response.responseJSON.phoneNumbers.map(v => {
                  return `${v.number}`;
                }).join(", ");
                finalList[index].capabilities = response.responseJSON.capabilities.map(v => {
                ...
                $.ajax({
                  ...
                  complete: function (orgResponse, orgTextStatus) {
                    if (orgTextStatus === 'success') {
                      finalList[index].zones = orgResponse.responseJSON.zones.map(v => {
                        finalList[index].clusters = v.clusters.map(v => {
                          return `${v.name}`;
                        }).join(", ");
                        return `${v.name}`;
                      }).join(", ");
